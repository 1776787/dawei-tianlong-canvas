...
export type AiConfig = {
    ...
    models: string[];
    quality: string;
    /** Optional per-node image resolution tier (auto/1k/2k/4k). */
    resolution?: "auto" | "1k" | "2k" | "4k" | string;
    size: string;
    background: string;
    count: string;
    canvasImageCount: string;
    /** Legacy compatibility field; RH requests always use the official API host. */
...
export const CONFIG_STORE_KEY = "dawei-tianlong-canvas:ai_config:v1";
...
export const defaultConfig: AiConfig = {
    ...
    count: "1",
    canvasImageCount: "3",
    runningHubBaseUrl: RUNNINGHUB_BASE_URL,
    runningHubApiKey: "",
};
            ...
            merge: (persisted, current) => {
                ...
                return {
                    ...
                    config: {
                        ...
                        runningHubBaseUrl: normalizeRunningHubBaseUrl(config.runningHubBaseUrl),
...
export function withChannels(config: AiConfig, channels: ModelChannel[]): AiConfig {
    ...
    for (const [key, capability] of [
        ["imageModel", "image"], ["videoModel", "video"], ["textModel", "text"], ["audioModel", "audio"],
    ] as const) {
        const options = selectableModelsByCapability(next, capability);
        const normalized = normalizeModelOptionValue(config[key], channels);
        next[key] = options.includes(normalized) ? normalized : options[0] || "";
    }
    return next;
}
