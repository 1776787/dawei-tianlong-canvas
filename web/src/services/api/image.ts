...
import { isApiProxyUrl, proxyApiUrl } from "@/lib/api-proxy";
import { normalizePluginImages, runModelPlugin } from "./model-plugin";
import { nanoid } from "nanoid";
import { dataUrlToFile } from "@/lib/image-utils";
import { buildImageReferencePromptText } from "@/lib/image-reference-prompt";
import { imageToDataUrl } from "@/services/image-storage";
import { imageDimensions, resolveImagePixelSize } from "@/lib/image-dimensions";
...
function resolveOpenAiImageParams(config: AiConfig, count: number) {
    ...
    const requestSize = resolveRequestSize(config);
...
/** Map resolution and ratio to an explicit pixel dimension like "3840x2160". */
function resolveSize(resolution: string | undefined, ratio: string): string {
    parseImageRatio(ratio);
    const dimensions = imageDimensions(ratio, resolution);
...
function resolveRequestSize(config: AiConfig) {
    const value = resolveImagePixelSize(config.size, config.resolution);
    ...
    if (value.includes(":")) return resolveSize(config.resolution, value);
...
function resolveGeminiImageConfig(config: AiConfig) {
    ...
    const imageSize = supportsGeminiImageSize(config.model) ? resolveGeminiImageSize(config.resolution || "auto", dimensions) : undefined;
...
function resolveGeminiImageSize(resolution: string, dimensions: { width: number; height: number } | null) {
    const imageSize = GEMINI_IMAGE_SIZE_BY_RESOLUTION[resolution];
...
function resolveImagePluginParams(config: AiConfig, count: number) {
    return {
        ...resolveOpenAiImageParams(config, count),
        count,
        ratio: config.size,
        resolution: config.resolution || "auto",
        geminiImageConfig: resolveGeminiImageConfig(config),
    };
}
...
function readAxiosError(error: unknown, fallback: string) {
    ...
    if (axios.isAxiosError(error)) {
        if (!error.response) {
            ...
            const hint = url ? `\n${url}` : "";
            return `${apiText(isApiProxyUrl(url) ? "proxyUnavailable" : "networkFailed")}${hint}`;
        }
        const responseData = error.response?.data;
        if (isApiProxyUrl(error.config?.url || "")) {
            const code = responseData?.error?.code;
            if (code === "proxy_timeout") return apiText("proxyTimeout");
            if (code === "proxy_upstream_failed") return `${apiText("proxyUpstreamFailed")}\n${readApiErrorMessage(responseData)}`;
        }
...
function consumeResponseStreamBlock(block: string, state: ResponseStreamState, onDelta?: (text: string) => void) {
    ...
    if (type === "response.incomplete" || type === "response.failed") state.error = errorMessage || "模型响应未完整完成，请检查已执行的操作后重试";
...
async function requestStreamingResponse(config: AiConfig, body: Record<string, unknown>, onDelta?: (text: string) => void, options?: RequestOptions): Promise<ToolResponseResult> {
    ...
    if (state.error) throw new Error(state.error);
    if (!state.payload) throw new Error("模型响应流提前结束，未收到完成事件");
...
function toGeminiContents(messages: ResponseInputMessage[]): GeminiContent[] {
    ...
    // Parallel calls and their responses belong to one model/user turn respectively.
    return contents.reduce<GeminiContent[]>((merged, current) => {
        const previous = merged.at(-1);
        if (previous && previous.role === current.role) previous.parts.push(...current.parts);
    ...
    if (script) {
        try {
            const result = await runModelPlugin({
                ...
                images: [],
                params: resolveImagePluginParams(requestConfig, n),
                signal: options?.signal,
            });
...
// Tool-calling chat for the canvas agent. Uses the text model and supports both
// the OpenAI Responses API and Gemini formats, reusing the streaming infra above.
export async function requestToolChat(
    config: AiConfig,
