...
type VideoResponse = { id: string; status?: string; state?: string; error?: { message?: string }; url?: string; result_url?: string; video_url?: string; content?: { video_url?: string; url?: string } | null };
    ...
    const result = videoPluginResult(
        await runModelPlugin({
            ...
            params: {
                ...
                quality: normalizeVideoQuality(config.quality),
                ratio: config.size,
...
async function createOpenAIVideoTask(config: AiConfig, model: string, prompt: string, references: ReferenceImage[], options?: RequestOptions): Promise<VideoGenerationTask> {
    ...
    body.append("quality", normalizeVideoQuality(config.quality));
...
async function pollOpenAIVideoTask(config: AiConfig, task: VideoGenerationTask, options?: RequestOptions): Promise<VideoGenerationTaskState> {
    try {
        ...
        if (video.status === "completed" || video.status === "done" || video.state === "completed") {
            const content = await axios.get<Blob>(aiApiUrl(config, `/videos/${task.id}/content`), { headers: aiHeaders(config), responseType: "blob", signal: options?.signal });
            await assertVideoBlob(content.data);
            return { status: "completed", result: { blob: content.data } };
        }
        if (["failed", "cancelled", "error"].includes(video.status || "") || ["failed", "cancelled"].includes(video.state || "")) return { status: "failed", error: readApiErrorMessage(video.error?.message) || apiText("videoGenerationFailed") };
...
function normalizeVideoResolution(value: string) {
    ...
    const resolution = normalized === "4k" ? "2160" : normalized === "2k" ? "1440" : normalized || "720";
    return `${resolution}p`;
}

function normalizeVideoQuality(value: string) {
    const normalized = value.trim().toLowerCase();
    return normalized === "high" || normalized === "medium" || normalized === "low" ? normalized : "auto";
}
