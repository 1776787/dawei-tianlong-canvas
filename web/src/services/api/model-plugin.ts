...
const imageParams = {};
for (const key of ["n", "size", "quality", "background", "output_format", "response_format"]) {
  if (params[key] !== undefined) imageParams[key] = params[key];
}
if (images.length === 0) {
...
for (const [key, value] of Object.entries(imageParams)) form.set(key, String(value));
...
const data = await request({
  ...
  data: { contents: [{ role: "user", parts }], generationConfig: { responseModalities: ["IMAGE"], ...params.geminiImageConfig } },
});
