...
import { normalizeRunningHubBaseUrl } from "@/lib/runninghub-config";
...
export async function importAppBackup(file: File) {
    ...
    useConfigStore.setState({ config: { ...data.config, runningHubBaseUrl: normalizeRunningHubBaseUrl(data.config.runningHubBaseUrl) } });
