...
export function buildGenerationConfig(config: AiConfig, node: CanvasNodeData | undefined, mode: CanvasNodeGenerationMode): AiConfig {
    const nodeResolution = node?.metadata?.resolution;
    const resolution = nodeResolution || config.resolution || defaultConfig.resolution;
    const quality = node?.metadata?.quality || config.quality || defaultConfig.quality;
    return {
        ...config,
...
export function resetInterruptedGeneration(nodes: CanvasNodeData[]) {
    return nodes.map((node) => {
        ...
        if (metadata?.status !== "loading" && !metadata?.images?.some(image => image.status === "loading")) return node;
        const images = metadata.images?.map(image => image.status === "loading" ? {
            ...image, status: "error" as const,
            errorDetails: i18n.t(image.pendingImageUrl ? "apiErrors.generatedImageReceiveFailed" : "canvas.generation.interrupted"),
        } : image);
        const hasResult = Boolean(metadata.content);
        return {
            ...node,
            metadata: {
                ...metadata, images,
                status: hasResult ? "success" as const : "error" as const,
                errorDetails: hasResult ? undefined : i18n.t(metadata.pendingImageUrl || images?.some(image => image.pendingImageUrl)
                    ? "apiErrors.generatedImageReceiveFailed" : "canvas.generation.interrupted"),
            },
        };
    });
}
