...
import { Clapperboard, Download, Ellipsis, FolderPlus, Image as ImageIcon, Info, MessageSquare, Minus, Music2, Plus, RefreshCw, Settings2, Trash2, Upload, Video } from "lucide-react";
...
export function CanvasNodeHoverToolbar({
    ...
    const nodeToolbarTools: ToolbarTool[] = [
        ...(canRetry ? [{
            id: "retry",
            title: t(node.metadata?.pendingImageUrl || node.metadata?.images?.some(image => image.pendingImageUrl) ? "canvas.node.receiveAgain" : "canvas.nodeToolbar.retryTitle"),
            label: t(node.metadata?.pendingImageUrl || node.metadata?.images?.some(image => image.pendingImageUrl) ? "canvas.node.receiveAgain" : "canvas.node.retry"),
            icon: <RefreshCw className="size-4" />, onClick: () => onRetry(node),
        }] : []),
