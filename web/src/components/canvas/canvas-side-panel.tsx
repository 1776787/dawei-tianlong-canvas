...
import { BookOpen, Check, ChevronRight, Clapperboard, Download, Eye, FileText, History as HistoryIcon, Image as ImageIcon, ListChecks, Music2, Plus, Search, Settings2, Square, Trash2, Type, Video, Workflow as WorkflowIcon } from "lucide-react";
...
const NODE_FILTER_VALUES = ["all", CanvasNodeType.Image, CanvasNodeType.Video, CanvasNodeType.Text, CanvasNodeType.Audio, CanvasNodeType.Config, CanvasNodeType.Group, RUNNINGHUB_TYPE];
                                ...
                                <div key={node.id} className={cn("group flex w-full items-center rounded-lg transition", active ? "" : "hover:bg-black/5 dark:hover:bg-white/5")} style={active ? { background: theme.toolbar.activeBg } : undefined}>
                                    <button type="button" onClick={() => (selectMode ? toggleChecked(node.id) : onFocusNode(node.id))} className="flex min-w-0 flex-1 items-center gap-3 px-2 py-2 text-left" title={selectMode ? undefined : t("canvas.sidePanel.focusNode")}>
                                        ...
                                        <span className="grid size-10 shrink-0 place-items-center overflow-hidden rounded-md">
                                            {isImage ? <CanvasPreviewImage src={node.metadata!.content!} alt={node.title} className="size-full object-cover" /> : <Icon className="size-5 opacity-60" />}
                                        </span>
...
function AssetCover({ asset }: { asset: Asset }) {
    ...
    return <CanvasPreviewImage src={asset.coverUrl || asset.data.dataUrl} alt="" className="size-full object-cover transition duration-300 group-hover:scale-[1.04]" />;
}
...
function HistoryThumb({ storageKey, theme }: { storageKey: string; theme: CanvasTheme }) {
    ...
    return <CanvasPreviewImage src={url} alt="" className="aspect-square w-full rounded-lg object-cover" loading="lazy" />;
}
