...
import { Brush, CircleDot, Clapperboard, Eraser, Film, Grid2x2, Group, Hand, Image as ImageIcon, Info, ListVideo, Moon, MousePointer2, Music2, Palette, Redo2, Settings2, Square, Sun, Trash2, Type, Undo2, Upload, Video, Workflow } from "lucide-react";
...
export function CanvasToolbar({
    ...
    // Close the appearance popover when clicking outside the toolbar and its panels.
    useEffect(() => {
    ...
    return (
        <div ref={rootRef} className="pointer-events-none absolute bottom-5 z-50 flex justify-center" style={{ left: 300, right: 16 }}>
            ...
            <div ref={wrapRef} className="thin-scrollbar pointer-events-auto flex h-14 max-w-full items-center gap-1 overflow-x-auto rounded-xl border px-2 shadow-lg backdrop-blur [&>*]:shrink-0" style={dockStyle}>
                ...
                <ToolbarButton id="tool-director" label={t("canvas.toolbar.director")} hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onAddDirector}>
                    <Clapperboard className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-storyboard" label={t("canvas.toolbar.storyboard")} hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onAddStoryboard}>
                    <ListVideo className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-shot-video" label={t("canvas.toolbar.shotVideo")} hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onAddShotVideo}>
                    <Film className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-doodle" label={t("canvas.toolbar.doodle")} hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onAddDoodle}>
                    <Brush className="size-4.5" />
                </ToolbarButton>
                <ToolbarButton id="tool-runninghub" label={t("canvas.toolbar.runninghub")} hovered={hovered} hoverStyle={hoverStyle} wrapRef={wrapRef} onTipX={setTipX} onHover={setHovered} onClick={onAddRunningHub}>
...
function toolLabel(id: string, t: (key: string) => string) {
    ...
    if (id === "tool-storyboard") return t("canvas.toolbar.storyboard");
    if (id === "tool-shot-video") return t("canvas.toolbar.shotVideo");
    if (id === "tool-doodle") return t("canvas.toolbar.doodle");
    if (id === "tool-runninghub") return t("canvas.toolbar.runninghub");
