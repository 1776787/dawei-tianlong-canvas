...
import { ChevronRight, Clapperboard, Copy, Group, Image as ImageIcon, Music2, Puzzle, RefreshCw, Star, Trash2, Video } from "lucide-react";
...
import { STORYBOARD_TYPE, StoryboardNodeContent } from "./nodes/storyboard-node";
import { SHOT_VIDEO_TYPE, ShotVideoNodeContent } from "./nodes/storyboard-video-node";
import { DOODLE_TYPE, DoodleNodeContent } from "./nodes/doodle-node";
import { RUNNINGHUB_TYPE } from "./nodes/runninghub-node";
...
export const CanvasNode = React.memo(function CanvasNode({
    ...
    const handleResizeMove = useCallback(
        (event: MouseEvent) => {
            ...
            const currentScale = getScale?.() ?? scale;
        ...
        [data.id, getScale, onResize, scale],
    );
    ...
    return (
        <div
            ...
            style={{
                ...
                willChange: hasImageContent ? "transform" : undefined,
            }}
            onMouseEnter={() => {
                setHovered(true);
...
function NodeContent(props: NodeContentRendererProps) {
    if ((props.node.type === CanvasNodeType.Config || props.node.type === SHOT_VIDEO_TYPE || props.node.type === RUNNINGHUB_TYPE) && props.renderNodeContent) return props.renderNodeContent(props.node);
    if (props.node.type === STORYBOARD_TYPE) return <StoryboardNodeContent node={props.node} theme={props.theme} />;
    if (props.node.type === SHOT_VIDEO_TYPE) return <ShotVideoNodeContent node={props.node} theme={props.theme} />;
    if (props.node.type === DOODLE_TYPE) return <DoodleNodeContent node={props.node} theme={props.theme} />;
    if (props.isBatchRoot) return <ImageNodeContent {...props} />;
    if (props.node.type === CanvasNodeType.Image && props.node.metadata?.content) return <ImageNodeContent {...props} />;
    if (props.node.metadata?.status === "loading") return <LoadingContent theme={props.theme} receiving={Boolean(props.node.metadata.pendingImageUrl || props.node.metadata.images?.some(image => image.pendingImageUrl && image.status === "loading"))} />;
...
function LoadingContent({ theme, receiving }: Pick<NodeContentRendererProps, "theme"> & { receiving?: boolean }) {
    const { t } = useTranslation();
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3" style={{ color: theme.node.activeStroke }}>
            <div className="size-10 animate-spin rounded-full border-2" style={{ borderColor: theme.node.stroke, borderTopColor: theme.node.activeStroke }} />
            <span className="px-3 text-center text-xs">{t(receiving ? "canvas.node.receivingImage" : "canvas.node.generating")}</span>
        </div>
    );
}
    ...
    return (
        <div className="flex max-w-[260px] flex-col items-center gap-3 px-5 text-center">
            ...
            >
                ...
                {t(node.metadata?.pendingImageUrl || node.metadata?.images?.some(image => image.pendingImageUrl) ? "canvas.node.receiveAgain" : "canvas.node.retry")}
            </button>
        </div>
    );
}
...
function ImageContent({
    ...
    return (
        <BatchFrame batchCount={batchCount} batchExpanded={batchExpanded}>
            ...
            {primaryImage?.status === "error" ? <BatchImageFailureActions placement="left" receiving={Boolean(primaryImage.pendingImageUrl)} onRetry={() => onRetryBatchImage?.(primaryImage.id)} onDelete={() => onDeleteBatchImage?.(primaryImage.id)} /> : null}
            {isBatchRoot ? (
                <button
                    type="button"
...
function ExpandedImageCard({
    ...
    return (
        ...
        >
            {image.content ? <CanvasPreviewImage src={image.content} alt={node.title} draggable={false} className="pointer-events-none h-full w-full select-none object-contain" /> : <ImageSlotStatus image={image} />}
            {image.content ? (
            ...
            {image.status === "error" ? <BatchImageFailureActions placement="right" receiving={Boolean(image.pendingImageUrl)} onRetry={onRetry} onDelete={onDelete} /> : null}
        </div>
    );
}

function BatchImageFailureActions({ placement, receiving, onRetry, onDelete }: { placement: "left" | "right"; receiving?: boolean; onRetry: () => void; onDelete: () => void }) {
    const theme = canvasThemes[useThemeStore((state) => state.theme)];
    const { t } = useTranslation();
    return (
        <div className={`absolute top-3 z-30 flex items-center gap-1.5 ${placement === "left" ? "left-3" : "right-3"}`}>
            ...
            >
                <RefreshCw className="size-3.5" />
                {t(receiving ? "canvas.node.receiveAgain" : "canvas.node.retry")}
            </button>
            <button
                type="button"
...
function ImageSlotStatus({ image }: { image?: CanvasNodeImage }) {
    ...
    return (
        <div className="flex h-full w-full flex-col items-center justify-center gap-3 px-6 text-center" style={{ background: theme.node.fill, color: failed ? theme.node.text : theme.node.activeStroke }}>
            ...
            {!failed ? <span className="text-center text-xs">{t(image?.pendingImageUrl ? "canvas.node.receivingImage" : "canvas.node.generating")}</span> : null}
        </div>
    );
}
