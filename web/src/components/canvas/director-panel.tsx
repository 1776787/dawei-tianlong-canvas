...
export function DirectorPanel({ nodeId, open, onClose, onExport }: DirectorPanelProps) {
    ...
    useEffect(() => {
        ...
        const handler = (event: MessageEvent) => {
            ...
            const frame = document.querySelector<HTMLIFrameElement>(`iframe[data-director-node="${CSS.escape(nodeId)}"]`);
    ...
    return (
        ...
        >
            <iframe data-director-node={nodeId} src={`${import.meta.env.BASE_URL}monoform/index.html?key=${encodeURIComponent(nodeId)}`} title="MONOFORM" className="h-full w-full border-0" allow="camera; microphone; clipboard-write; download; fullscreen" />
        </Modal>
    );
}
