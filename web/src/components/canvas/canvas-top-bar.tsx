...
import { Download, Home, Images, Magnet, Menu, PanelLeftClose, PanelLeftOpen, Plus, Redo2, Trash2, Undo2, Upload } from "lucide-react";
...
export function CanvasTopBar({
    ...
    onAlignChange: (enabled: boolean) => void;
}) {
        ...
        <>
            <div className="pointer-events-none absolute left-0 right-0 top-0 z-50 flex h-16 items-center justify-between pl-1 pr-4">
                ...
                <div className="pointer-events-auto flex items-center gap-1.5">
                    <Tooltip title={t(alignEnabled ? "canvas.alignment.disable" : "canvas.alignment.enable")}>
                        <span className="inline-flex items-center gap-1.5 rounded-full px-2 py-1" style={{ color: theme.node.text, background: theme.toolbar.panel }}>
                            <Magnet className="size-3.5" />
                            <Switch size="small" checked={alignEnabled} onChange={onAlignChange} aria-label={t("canvas.alignment.toggle")} />
                        </span>
                    </Tooltip>
