...
function readReferenceImage(node: CanvasNodeData): ReferenceImage | null {
    // Built-in image nodes, plus plugin nodes (e.g. doodle) that declare an image resource.
    const isImageResource = node.type === CanvasNodeType.Image || getNodeDefinition(node.type)?.resource?.(node)?.kind === "image";
    if (!isImageResource || !node.metadata?.content) return null;
    return {
        id: node.id,
