...
type VideoSettingsPanelProps = {
    config: AiConfig;
    onConfigChange: (key: "quality" | "vquality" | "size" | "videoSeconds" | "videoGenerateAudio" | "videoWatermark", value: string) => void;
    theme: CanvasTheme;
    showTitle?: boolean;
    className?: string;
};
    ...
    return (
        <ImageSettingsTheme theme={theme}>
            <div className={className} style={{ color: theme.node.text }} onMouseDown={(event) => event.stopPropagation()}>
                ...
                <SettingGroup title="Quality" color={theme.node.muted}>
                ...
                <SettingGroup title="Resolution" color={theme.node.muted}>
                ...
                <div className="flex items-center justify-between gap-3">
                    <div className="space-y-0.5">
                        <SettingGroupTitle color={theme.node.muted}>{t("settingsPanels.video.generateAudio")}</SettingGroupTitle>
                        <div className="text-xs opacity-60">{t("settingsPanels.video.generateAudioHint")}</div>
                    </div>
                    <span onMouseDown={(event) => event.stopPropagation()}>
                        <Switch size="small" checked={config.videoGenerateAudio !== "false"} onChange={(checked) => onConfigChange("videoGenerateAudio", String(checked))} />
                    </span>
                </div>
                <div className="flex items-center justify-between gap-3">
                    <div className="space-y-0.5">
                        <SettingGroupTitle color={theme.node.muted}>{t("settingsPanels.video.watermark")}</SettingGroupTitle>
                        <div className="text-xs opacity-60">{t("settingsPanels.video.watermarkHint")}</div>
                    </div>
                    <span onMouseDown={(event) => event.stopPropagation()}>
                        <Switch size="small" checked={config.videoWatermark === "true"} onChange={(checked) => onConfigChange("videoWatermark", String(checked))} />
                    </span>
                </div>
            </div>
        </ImageSettingsTheme>
...
export function normalizeVideoResolutionValue(value: string) {
    ...
    const normalized = value.replace(/p$/i, "").toLowerCase();
...
function SettingGroupTitle({ children, color }: { children: string; color: string }) {
    return (
        <div className="text-xs font-medium" style={{ color }}>
            {children}
        </div>
...
function NumberInput({ value, min, max, theme, onChange }: { value: string; min: number; max: number; theme: CanvasTheme; onChange: (value: string) => void }) {
    const normalize = (raw: string) => {
        ...
        return Number.isFinite(parsed) ? String(Math.max(min, Math.min(max, parsed))) : value;
    };
    return (
        <label
            className="flex h-9 min-w-0 overflow-hidden rounded-full border text-sm"
            style={{ borderColor: theme.node.stroke, color: theme.node.text }}
            title={`Custom duration (${min}-${max}s)`}
            onMouseDown={(event) => event.stopPropagation()}
        >
            <input
            ...
            />
            <span className="grid w-7 place-items-center pr-1 text-xs opacity-60">s</span>
        </label>
    );
}
