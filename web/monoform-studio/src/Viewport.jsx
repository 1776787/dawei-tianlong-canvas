...
function PlantModel({ color }) {
  return <group><group position={[0, -0.38, 0]} scale={[0.34, 0.26, 0.34]}><SimplePart shape="cylinder" color="#8a8378" /></group><group position={[0, -0.1, 0]} scale={[0.08, 0.4, 0.08]}><SimplePart shape="cylinder" color="#766b57" /></group><group position={[0, 0.2, 0]} scale={[0.6, 0.5, 0.6]}><SimplePart shape="sphere" color={color} /></group><group position={[-0.2, 0.02, 0.12]} scale={[0.34, 0.3, 0.34]}><SimplePart shape="sphere" color={color} /></group><group position={[0.22, 0.05, -0.1]} scale={[0.3, 0.28, 0.3]}><SimplePart shape="sphere" color={color} /></group></group>
}

function BuildingModel({ color }) {
  return <group><group scale={[1, 1, 1]}><SimplePart color={color} /></group><group position={[0, 0.53, 0]} scale={[0.72, 0.06, 0.76]}><SimplePart color="#8b877e" /></group><group position={[0.18, 0.6, 0.1]} scale={[0.2, 0.12, 0.2]}><SimplePart color="#8b877e" /></group>{[-0.28, 0, 0.28].flatMap(x => [-0.3, -0.06, 0.18].map(y => <group key={`${x}-${y}`} position={[x, y, 0.505]} scale={[0.16, 0.14, 0.02]}><SimplePart color="#6f6c66" /></group>))}<group position={[0, -0.42, 0.505]} scale={[0.2, 0.16, 0.02]}><SimplePart color="#54524d" /></group></group>
}

function RoadModel({ color }) {
  return <group><group scale={[1, 1, 1]}><SimplePart color={color} /></group>{[-0.4, -0.2, 0, 0.2, 0.4].map(x => <group key={x} position={[x, 0.52, 0]} scale={[0.08, 0.08, 0.02]}><SimplePart color="#e8e3d8" /></group>)}</group>
}
...
function PrimitiveModel({ type, color, selected, parts = [] }) {
  ...
  if (type === 'plant') return <PlantModel color={materialColor} />
  if (type === 'building') return <BuildingModel color={materialColor} />
  if (type === 'road') return <RoadModel color={materialColor} />
...
function CameraModel({ data, selected, selectedId, transformMode, transformSpace = 'world', snapEnabled = true, onSelect, onUpdate }) {
  ...
  useLayoutEffect(() => {
    ...
    groupRef.current.rotation.set(...data.rotation, data.rotationOrder || CAMERA_ROTATION_ORDER)
  }, [data.position, data.rotation, data.rotationOrder])
...
function EditorScene({ objects, selectedId, activeJoint, onSelect, onJointSelect, transformMode, transformSpace, snapEnabled, groundRequest, onUpdateObject, cameraData, cameraAspect, onUpdateCamera, editorCameraData, onEditorCameraChange, lighting, showGrid, focusRequest, referenceVisible = false, cameraView = false, cameraLocked = false, animationTime = 0 }) {
  return (
    <>
      ...
      {cameraView ? <><PreviewCameraController cameraData={cameraData} cameraAspect={cameraAspect} /><ShotViewDrag cameraData={cameraData} onChange={onUpdateCamera} locked={cameraLocked} /></> : <OrbitControls makeDefault target={editorCameraData?.target || [0, 1, 0]} minDistance={2} maxDistance={35} maxPolarAngle={Math.PI * 0.49} />}
...
function ShotViewDrag({ cameraData, onChange, locked = false }) {
  const { gl } = useThree()
  const latest = useRef({ cameraData, onChange })
  latest.current = { cameraData, onChange }
  useEffect(() => {
    ...
    const down = event => {
      ...
      drag = { id: event.pointerId, x: event.clientX, y: event.clientY, rotation: [...latest.current.cameraData.rotation] }
      element.setPointerCapture(event.pointerId)
    }
    const move = event => {
      if (!drag || drag.id !== event.pointerId) return
      event.stopImmediatePropagation()
      const [pitch, yaw, roll] = drag.rotation
      latest.current.onChange({ rotation: [THREE.MathUtils.clamp(pitch - (event.clientY - drag.y) * 0.004, -Math.PI / 2 + 0.01, Math.PI / 2 - 0.01), yaw - (event.clientX - drag.x) * 0.004, roll], rotationOrder: CAMERA_ROTATION_ORDER })
    }
    const up = event => {
      if (!drag || drag.id !== event.pointerId) return
      drag = null
...
function PreviewCameraController({ cameraData, cameraAspect }) {
  ...
  useFrame(() => {
    ...
    camera.rotation.set(...cameraData.rotation, cameraData.rotationOrder || CAMERA_ROTATION_ORDER)
...
export function MainViewport(props) {
  ...
  const cameraSettings = props.cameraView
    ? { position: props.cameraData.position, fov: 42, near: 0.05, far: 200 }
    : {
