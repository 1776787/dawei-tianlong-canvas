...
import { CameraPathPanel } from './CameraPathPanel.jsx'
...
// 预设机位（对齐桥豆麻衣酱 3D 导演台）：相对选中人物（或场景中第一个人物）计算机位。
const CAMERA_PRESETS = [
  { id: 'front-medium', label: '正面中景', azimuth: 0, distance: 3.2, height: 1.35, focal: 42 },
  { id: 'front-closeup', label: '正面特写', azimuth: 0, distance: 1.4, height: 1.5, focal: 85, lookHeight: 1.55 },
  { id: 'front-full', label: '正面全景', azimuth: 0, distance: 6.5, height: 1.6, focal: 35 },
  { id: 'side-track', label: '侧面跟拍', azimuth: 90, distance: 3.4, height: 1.4, focal: 42 },
  { id: 'side-close', label: '侧面近景', azimuth: 90, distance: 1.8, height: 1.45, focal: 60, lookHeight: 1.5 },
  { id: 'back-medium', label: '背面中景', azimuth: 180, distance: 3.2, height: 1.45, focal: 42 },
  { id: 'high-full', label: '俯拍全景', azimuth: 0, distance: 7, height: 5.5, focal: 35, lookHeight: 1 },
  { id: 'high-45', label: '45° 俯拍', azimuth: 45, distance: 4.2, height: 3.6, focal: 42, lookHeight: 1.1 },
  { id: 'low-up', label: '低角度仰拍', azimuth: 0, distance: 2.6, height: 0.45, focal: 42, lookHeight: 1.6 },
  { id: 'low-wide', label: '低角度广角', azimuth: 0, distance: 2, height: 0.35, focal: 18, lookHeight: 1.5 },
  { id: 'ots-left', label: '过肩镜头', azimuth: 152, distance: 1.7, height: 1.62, focal: 50, lookHeight: 1.45 },
  { id: 'ots-right', label: '过肩镜头（右）', azimuth: -152, distance: 1.7, height: 1.62, focal: 50, lookHeight: 1.45 },
  { id: 'birdeye', label: '鸟瞰', azimuth: 0, distance: 0.8, height: 9, focal: 28, lookHeight: 0 },
  { id: 'dutch', label: '荷兰角', azimuth: 25, distance: 2.8, height: 1.4, focal: 42, roll: 14 },
]
...
const initialObjects = [
  {
    id: 'actor-lead', name: '人物 · 主角', type: 'person', bodyType: 'standard', pose: normalizePoseId('idle'),
    poseTime: 0, continuousMotion: false, position: [0, 0, 0], rotation: [0, 0, 0], scale: [1, 1, 1], color: '#e8e3d8', joints: presetJoints(),
  },
]
...
const initialCamera = {
  position: [0, 1.55, 2.15],
  rotation: cameraRotationToward([0, 1.55, 2.15], [0, 1.55, 0]),
  rotationOrder: CAMERA_ROTATION_ORDER,
  focalLength: 85,
  aspectRatio: '16:9',
}
...
function cameraRotationFromDirection(direction, roll = 0) {
  let [dx, dy, dz] = direction
  const length = Math.hypot(dx, dy, dz)
  if (length < 1e-8) return [0, 0, roll]
  dx /= length; dy /= length; dz /= length
  return [Math.asin(Math.min(1, Math.max(-1, dy))), Math.atan2(-dx, -dz), roll]
}

// 相机使用 YXZ：先水平转向，再俯仰，Z 才是独立翻滚；这样水平线不会随水平角倾斜。
function cameraRotationToward(position, target) {
  ...
  return cameraRotationFromDirection([point[0] - eye[0], point[1] - eye[1], point[2] - eye[2]])
}

function normalizeStoredCameraRotation(position, rotation, target, fallback, rotationOrder) {
  if (!Array.isArray(rotation)) return cameraRotationToward(position, target)
  const stored = finiteVector3(rotation, fallback)
  if (rotationOrder === CAMERA_ROTATION_ORDER) return stored
  const direction = new THREE.Vector3(0, 0, -1).applyEuler(new THREE.Euler(...stored, 'XYZ'))
  return cameraRotationFromDirection(direction.toArray(), stored[2])
}

function normalizeCamera(camera = {}) {
  const position = finiteVector3(camera.position, initialCamera.position)
  const target = camera.target || LEGACY_DEFAULT_CAMERA_TARGET
  const rotation = normalizeStoredCameraRotation(position, camera.rotation, target, initialCamera.rotation, camera.rotationOrder)
  return {
    position,
    rotation,
    rotationOrder: CAMERA_ROTATION_ORDER,
...
function normalizeCameraKeyframes(keys = [], fallbackCamera = initialCamera) {
  ...
  return uniqueSortedKeyframes(source.map(key => {
    ...
    const target = key.target || LEGACY_DEFAULT_CAMERA_TARGET
    return {
      frame: normalizeFrameNumber(key.frame),
      interpolation: normalizeInterpolation(key.interpolation),
      position,
      rotation: normalizeStoredCameraRotation(position, key.rotation, target, fallbackCamera.rotation, key.rotationOrder),
      rotationOrder: CAMERA_ROTATION_ORDER,
...
function readCachedProject() {
  try {
    ...
    const legacy = current || EMBED_KEY ? null : localStorage.getItem(LEGACY_PROJECT_STORAGE_KEY)
...
function LeftSidebar({ objects, selectedId, onSelect, onAddPerson, onAddPrimitive, onImport, onToggleVisible, onToggleLock, onApplyCameraPreset, shots, activeShotId, onSelectShot, onAddShot, onDuplicateShot, onDeleteShot, onRenameShot, onCaptureShot }) {
      ...
      {tab === 'assets' ? (
        <div className="assets-scroll">
          ...
          <div className="primitive-grid blockout-grid">
            {[['arch', '拱门'], ['stairs', '楼梯'], ['door', '门'], ['window', '窗'], ['table', '桌子'], ['chair', '椅子'], ['sofa', '沙发'], ['roof', '屋顶'], ['tree', '树木'], ['vehicle', '车辆'], ['plant', '植物'], ['building', '建筑'], ['road', '道路']].map(([type, label]) => (
              <button key={type} onClick={() => onAddPrimitive(type)}><Box size={20} /><span>{label}</span></button>
            ))}
          </div>
          <div className="section-kicker section-gap">预设机位</div>
          <div className="primitive-grid blockout-grid camera-preset-grid">
            {CAMERA_PRESETS.map(preset => (
              <button key={preset.id} onClick={() => onApplyCameraPreset(preset)} title={'应用机位：' + preset.label}><Camera size={20} /><span>{preset.label}</span></button>
            ))}
          </div>
...
function Inspector({ selected, camera, selectedJoint, customPoses, onSelectJoint, onUpdateObject, onUpdateCamera, onDelete, onDuplicate, onFocus, onToggleLock, onGround, onResetRotation, onResetScale, onSaveCustomPose, onApplyCustomPose, onDeleteCustomPose }) {
  ...
  const applyPreset = pose => {
    const normalizedPose = normalizePoseId(pose)
    onUpdateObject({
      pose: normalizedPose,
      poseTime: presetPhase(normalizedPose),
      continuousMotion: poseCanLoop(normalizedPose),
      rigRoot: presetRoot(normalizedPose),
      joints: presetJoints(normalizedPose),
    })
  }
  return (
...
export default function App() {
  ...
  // 新导演台默认直接进入摄像机视角，因此先选中主摄像机；已有工程仍保留编辑视角的原选择。
  const [selectedId, setSelectedId] = useState(() => startupProject ? (startupProject.objects?.[0]?.id || CAMERA_ID) : CAMERA_ID)
  const [selectedJoint, setSelectedJoint] = useState('mixamorigSpine2')
  const [transformMode, setTransformMode] = useState('translate')
  const pressedMoveKeysRef = useRef(new Set())
  const walkingPersonRef = useRef(null)
  const moveShiftRef = useRef(false)
  const [moveActive, setMoveActive] = useState(false)
  const selectedIdRef = useRef(null)
  const cameraStateRef = useRef(null)
  const cameraViewRef = useRef(false)
  const objectsRef = useRef([])
  const updateObjectByIdRef = useRef(null)
  ...
  const [cameraView, setCameraView] = useState(!startupProject)
  ...
  // 空闲动画时钟：时间轴未播放时，持续动作（行走/奔跑等循环）也实时预演。
  const [idleClock, setIdleClock] = useState(0)
  const idleClockRef = useRef(0)
  const hasLoopMotion = objects.some(object => object.type === 'person' && object.continuousMotion && poseCanLoop(object.pose))
  useEffect(() => {
    if (playing || !hasLoopMotion) return
    let frameId
    const start = performance.now() - idleClockRef.current * 1000
    const tick = timestamp => {
      idleClockRef.current = (timestamp - start) / 1000
      setIdleClock(idleClockRef.current)
      frameId = requestAnimationFrame(tick)
    }
    frameId = requestAnimationFrame(tick)
    return () => cancelAnimationFrame(frameId)
  }, [playing, hasLoopMotion])
  const liveAnimationTime = playing ? currentFrame / fps : (hasLoopMotion ? idleClock : currentFrame / fps)
  ...
  useEffect(() => {
    if (!EMBED_KEY) return
    const receive = event => {
      if (event.origin !== window.location.origin || event.source !== window.parent || event.data?.source !== 'canvas-director') return
      const { requestId, type, project } = event.data
      if (exportLockRef.current && type === 'set-project') {
        window.parent.postMessage({ source: 'monoform-director', requestId, error: '正在导出，请稍后更新场景' }, window.location.origin)
        return
      }
      if (type === 'get-project') window.parent.postMessage({ source: 'monoform-director', requestId, project: latestProjectRef.current }, window.location.origin)
      if (type === 'set-project') {
        const normalized = normalizeProjectData(project)
        if (!normalized) { window.parent.postMessage({ source: 'monoform-director', requestId, error: '场景数据无效' }, window.location.origin); return }
        latestProjectRef.current = normalized
        applyProjectSnapshot(normalized)
        setCameraView(true)
        setSelectedId(CAMERA_ID)
        setCurrentFrame(0)
        currentFrameRef.current = 0
        window.parent.postMessage({ source: 'monoform-director', requestId, project: normalized }, window.location.origin)
      }
    }
  ...
  useEffect(() => {
    ...
    updateObjectByIdRef.current = updateObjectById
  })

  // 按住即每帧移动（rAF 驱动，无键盘重复延迟）：人物行走/奔跑，机位观察者式飞行。
  useEffect(() => {
    ...
    const tick = now => {
      ...
      if (keys.size && sid) {
        ...
        if (keys.has('e')) moveUp += 1
        if (keys.has('q')) moveUp -= 1
        const running = moveShiftRef.current
        let yaw = 0, pitch = 0
        if (cameraViewRef.current) {
          yaw = cameraStateRef.current?.rotation?.[1] || 0
          pitch = cameraStateRef.current?.rotation?.[0] || 0
        } else {
          const view = editorViewRef.current
          if (view?.position && view?.target) {
            const vx = view.target[0] - view.position[0]
            const vy = view.target[1] - view.position[1]
            const vz = view.target[2] - view.position[2]
            const len = Math.hypot(vx, vy, vz) || 1
            yaw = Math.atan2(-vx, -vz)
            pitch = Math.asin(Math.max(-1, Math.min(1, vy / len)))
          }
        }
        if (sid === CAMERA_ID) {
          const speed = (running ? 16 : 6) * dt
          const forward = [-Math.sin(yaw) * Math.cos(pitch), Math.sin(pitch), -Math.cos(yaw) * Math.cos(pitch)]
          const right = [Math.cos(yaw), 0, -Math.sin(yaw)]
          const dx = (forward[0] * moveForward + right[0] * moveRight) * speed
          const dy = (forward[1] * moveForward + moveUp) * speed
          const dz = (forward[2] * moveForward + right[2] * moveRight) * speed
          setCamera(current => ({ ...current, position: [current.position[0] + dx, Math.max(0.05, current.position[1] + dy), current.position[2] + dz] }))
        } else {
          const target = objectsRef.current.find(object => object.id === sid)
          if (target && !target.locked) {
            const speed = (running ? 4 : 1.6) * dt
            const dx = (-Math.sin(yaw) * moveForward + Math.cos(yaw) * moveRight) * speed
            const dz = (-Math.cos(yaw) * moveForward - Math.sin(yaw) * moveRight) * speed
            const dy = moveUp * speed
            const patch = { position: [target.position[0] + dx, target.position[1] + dy, target.position[2] + dz] }
            if (target.type === 'person' && (moveForward || moveRight)) {
              patch.rotation = [0, Math.atan2(dx, dz), 0]
              const desiredPose = running ? 'run' : 'walk'
              if (normalizePoseId(target.pose) !== desiredPose || !target.continuousMotion) {
                patch.pose = desiredPose
                patch.poseTime = presetPhase(desiredPose)
                patch.joints = presetJoints(desiredPose)
                patch.continuousMotion = true
                walkingPersonRef.current = target.id
              }
            }
            updateObjectByIdRef.current?.(sid, patch)
          }
        }
      }
      frameId = requestAnimationFrame(tick)
  ...
  useEffect(() => {
    // 游戏式移动（对齐桥豆麻衣酱导演台）：
    // - 人物：WASD 相对视角行走（自动转向移动方向 + 行走循环，Shift = 奔跑），松开回到站立；Q/E 降升。
    // - 机位：MC 观察者式飞行——W/S 沿视线（含俯仰）前后，A/D 平移，Q/E 降升，Shift 加速。
    const MOVE_VECTORS = { w: [0, 0, 1], s: [0, 0, -1], a: [-1, 0, 0], d: [1, 0, 0], q: [0, -1, 0], e: [0, 1, 0] }
    const onKeyDown = event => {
      if (['INPUT', 'SELECT', 'TEXTAREA'].includes(document.activeElement?.tagName)) return
      const key = event.key.toLowerCase()
      moveShiftRef.current = event.shiftKey
      if (MOVE_VECTORS[key] && !event.ctrlKey && !event.metaKey && !event.altKey && selectedId) {
        const target = selectedId === CAMERA_ID ? null : objects.find(object => object.id === selectedId)
        if (selectedId === CAMERA_ID || (target && !target.locked)) {
          event.preventDefault()
      ...
      if (key === '1') setTransformMode('select')
      if (key === '2') setTransformMode('translate')
      if (key === '3') setTransformMode('rotate')
      if (key === '4') setTransformMode('scale')
      if (key === 'f') focusSelected()
      if (event.code === 'Space') { event.preventDefault(); togglePlayback() }
      if ((event.key === 'Delete' || event.key === 'Backspace') && selectedId !== CAMERA_ID) deleteSelected()
      if ((event.ctrlKey || event.metaKey) && key === 'd') { event.preventDefault(); duplicateSelected() }
      if ((event.ctrlKey || event.metaKey) && !event.shiftKey && key === 'z') { event.preventDefault(); undo() }
      if ((event.ctrlKey || event.metaKey) && (key === 'y' || (event.shiftKey && key === 'z'))) { event.preventDefault(); redo() }
    }
    const stopMoving = () => {
      pressedMoveKeysRef.current.clear()
      setMoveActive(false)
      const walkerId = walkingPersonRef.current
      if (!walkerId) return
      walkingPersonRef.current = null
      const walker = objectsRef.current.find(object => object.id === walkerId)
      if (walker && walker.type === 'person') {
        updateObjectByIdRef.current?.(walkerId, { pose: 'idle', poseTime: presetPhase('idle'), joints: presetJoints('idle'), continuousMotion: false })
      }
    }
    const onKeyUp = event => {
      const key = event.key.toLowerCase()
      moveShiftRef.current = event.shiftKey
      if (!MOVE_VECTORS[key]) return
      pressedMoveKeysRef.current.delete(key)
      if (!pressedMoveKeysRef.current.size) stopMoving()
    }
    const onBlur = () => stopMoving()
  ...
  const applyShotState = shot => {
    ...
    setSelectedId(cameraView ? CAMERA_ID : (shot.objects?.[0]?.id || CAMERA_ID))
  }
  ...
  const addPrimitive = type => {
    const id = uid()
    const labels = { box: '方块', sphere: '球体', cylinder: '圆柱', plane: '平面', arch: '拱门', stairs: '楼梯', table: '桌子', chair: '椅子', sofa: '沙发', door: '门', window: '窗', tree: '树木', vehicle: '车辆', roof: '屋顶', plant: '植物', building: '建筑', road: '道路' }
    const defaultScales = { arch: [1.8, 2.2, 0.45], stairs: [2.2, 1.4, 2.8], door: [1.2, 2.2, 0.25], window: [1.5, 1.3, 0.22], table: [1.7, 1, 1.1], chair: [0.8, 1, 0.8], sofa: [2.2, 1.1, 1], tree: [1.8, 2.6, 1.8], vehicle: [2.8, 1.2, 1.6], roof: [2.8, 1.2, 2.2], plant: [0.7, 1.1, 0.7], building: [4.2, 5.2, 3.4], road: [8, 0.06, 2.4] }
    const positionY = type === 'plane' || type === 'road' ? 0.03 : (type === 'tree' ? 1.3 : type === 'plant' ? 0.55 : type === 'building' ? 2.6 : 0.5)
    setObjects(list => [...list, { id, name: `${labels[type] || type} · ${list.filter(item => item.type === type).length + 1}`, type, position: [0, positionY, 0], rotation: [0, 0, 0], scale: type === 'plane' ? [2, 1, 2] : (defaultScales[type] || [1, 1, 1]), color: type === 'tree' || type === 'plant' ? '#9ca68d' : type === 'road' ? '#6f6c66' : '#c7c2b7' }])
    setSelectedId(id)
  }
  const applyCameraPreset = preset => {
    // 机位相对选中人物（或第一个人物、否则原点）计算；正面 = 人物面前。
    const anchorObject = (activeObject && activeObject.type === 'person' ? activeObject : null) || objects.find(object => object.type === 'person') || null
    const base = anchorObject ? finiteVector3(anchorObject.position, [0, 0, 0]) : [0, 0, 0]
    const yaw = (anchorObject?.rotation?.[1] || 0) + (preset.azimuth || 0) * Math.PI / 180
    const distance = preset.distance || 3
    // 人物白模面向 +Z（鼻子在 +Z），正面机位应位于人物面向方向上。
    const position = [base[0] + Math.sin(yaw) * distance, Math.max(0.08, base[1] + (preset.height ?? 1.4)), base[2] + Math.cos(yaw) * distance]
    const look = [base[0], base[1] + (preset.lookHeight ?? 1.35), base[2]]
    const rotation = cameraRotationToward(position, look)
    if (preset.roll) rotation[2] = preset.roll * Math.PI / 180
    setCamera(current => ({ ...current, position, rotation, rotationOrder: CAMERA_ROTATION_ORDER, focalLength: preset.focal || current.focalLength }))
    setSelectedId(CAMERA_ID)
    setCameraView(true)
    setToast('已应用机位：' + preset.label)
  }
  ...
  const addKeyframe = () => {
    ...
    const next = { frame: currentFrame, interpolation: normalizeInterpolation(existing?.interpolation), position: [...camera.position], rotation: [...camera.rotation], rotationOrder: CAMERA_ROTATION_ORDER, focalLength: camera.focalLength }
    ...
    <main className="app-shell" aria-busy={exporting || capturingImage}>
      <header className="topbar">
        ...
        <div className="export-actions">
          ...
          <button onClick={() => setPathOpen(value => !value)} aria-pressed={pathOpen}><Camera size={14} />相机路径</button>
        </div>
      </header>

      {pathOpen && <CameraPathPanel keys={keyframes} camera={camera} fps={fps} totalFrames={totalFrames} busy={exporting || capturingImage || playing} onChange={setKeyframes} onSeek={seekToFrame} onPreview={() => { setCurrentFrame(0); currentFrameRef.current = 0; setSelectedId(CAMERA_ID); setCameraView(true); setPlaying(true) }} onRecord={handleExportMp4} />}
      <div className="workspace">
        <LeftSidebar objects={objects} selectedId={selectedId} onSelect={setSelectedId} onAddPerson={addPerson} onAddPrimitive={addPrimitive} onImport={importModel} onApplyCameraPreset={applyCameraPreset} onToggleVisible={id => updateObjectById(id, { visible: objects.find(item => item.id === id)?.visible === false })} onToggleLock={id => updateObjectById(id, { locked: !objects.find(item => item.id === id)?.locked })} shots={displayedShots} activeShotId={activeShotId} onSelectShot={switchShot} onAddShot={addShot} onDuplicateShot={duplicateShot} onDeleteShot={deleteShot} onRenameShot={renameShot} onCaptureShot={captureShotThumbnail} />
        <section className="viewport-shell">
          <div className="viewport-toolbar floating-panel">
            <ToolButton icon={MousePointer2} label="选择" active={!['translate', 'rotate', 'scale'].includes(transformMode)} onClick={() => setTransformMode('select')} shortcut="1" />
            <span />
            <ToolButton icon={Move3D} label="移动" active={transformMode === 'translate'} onClick={() => setTransformMode('translate')} shortcut="2" />
            <ToolButton icon={RotateCw} label="旋转" active={transformMode === 'rotate'} onClick={() => setTransformMode('rotate')} shortcut="3" />
            <ToolButton icon={BoxSelect} label="缩放" active={transformMode === 'scale'} onClick={() => setTransformMode('scale')} shortcut="4" />
            <span />
          ...
          <div className="viewport-mode-help">
            {transformMode === 'select' && '1 选择 · 人物 WASD 行走（Shift 奔跑）· 机位 WASD 观察者飞行 · Q/E 降升'}
            {transformMode === 'translate' && `2 整体移动 · WASD 平移选中 · Q/E 降升 · ${transformSpace === 'world' ? '世界坐标' : '局部坐标'} · ${snapEnabled ? '0.1 格吸附' : '自由移动'}`}
            {transformMode === 'rotate' && `${selectedId === CAMERA_ID ? '3 摄像机旋转' : '3 整体旋转'} · ${transformSpace === 'world' ? '世界坐标' : '局部坐标'} · ${snapEnabled ? '5° 吸附' : '自由旋转'}`}
            {transformMode === 'scale' && `4 整体缩放 · ${transformSpace === 'world' ? '世界坐标' : '局部坐标'} · ${snapEnabled ? '0.1 吸附' : '自由缩放'}`}
          </div>
          ...
          <div className={`camera-monitor is-${monitorMode}`}>
            ...
            {monitorMode !== 'minimized' && <div className="monitor-frame">
              <div className={`monitor-canvas ${previewAspectClass}`} style={{ '--preview-aspect': previewAspect }}>
                <CameraPreview objects={animatedObjects} animationTime={liveAnimationTime} cameraData={displayCamera} cameraAspect={previewAspect} lighting={lighting} backgroundCanvas={monitorReferenceBackground} onCanvasReady={canvas => { monitorCanvasRef.current = canvas }} />
